...
// ===== ОПТИМИЗАЦИЯ ЗОЛОТОГО ТЕКСТА =====

function optimizeGoldText() {
    const goldTitles = document.querySelectorAll('.gold-title');
    ...
    goldTitles.forEach(title => {
        ...
        if (window.innerWidth <= 768) {
            ...
            const goldTexts = title.querySelectorAll('.gold-text');
        ...
        // Отключаем сложные анимации при уменьшенном движении
...
// ===== ОПТИМИЗАЦИЯ СЛАЙДЕРА БЕЗ ИНДИКАТОРОВ =====
...
class OptimizedHeroSlider {
    ...
    showSlide(index) {
        ...
        // Оптимизация анимации для текущего слайда
        this.optimizeCurrentSlide();
    }
    ...
    optimizeForMobile() {
        if (window.innerWidth <= 768) {
            // Увеличиваем задержку автоплея на мобильных
            this.autoPlayDelay = 10000;
            
            // Перезапускаем автоплей с новой задержкой
            this.resetAutoPlay();
    ...
    optimizeCurrentSlide() {
        const activeSlide = this.slides[this.currentSlide];
        const goldTitle = activeSlide.querySelector('.gold-title');
        
        if (goldTitle) {
            // Добавляем ленивую загрузку для следующего слайда
            const nextIndex = (this.currentSlide + 1) % this.totalSlides;
            const nextSlide = this.slides[nextIndex];
            
            if (nextSlide) {
                const nextBackground = nextSlide.querySelector('.slide-background');
                if (nextBackground) {
                    const bgImage = window.getComputedStyle(nextBackground).backgroundImage;
...
document.addEventListener('DOMContentLoaded', () => {
    // Инициализация оптимизированного слайдера
    new OptimizedHeroSlider();
    
    // Оптимизация золотого текста
    optimizeGoldText();
    
    // Оптимизация при изменении размера окна
    window.addEventListener('resize', optimizeGoldText);
...
// Проверка поддержки WebGL для сложных эффектов
...
// Отключение сложных эффектов если WebGL не поддерживается
if (!checkWebGLSupport()) {
    ...
    style.textContent = `
        ...
        .gold-text {
            animation: goldShimmerSimple 3s ease-in-out infinite alternate !important;
        }
        
        @keyframes goldShimmerSimple {
