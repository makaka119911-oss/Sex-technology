...
/** Секрет админки из Vercel (основное имя или запасное). */
export function getOrderAdminSecret(): string | undefined {
  const a = process.env.ORDER_ADMIN_SECRET?.trim()
  const b = process.env.SHOP_ADMIN_SECRET?.trim()
